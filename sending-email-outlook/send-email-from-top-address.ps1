...
try {
    ...
    # ── Workaround: Outlook always sends from the first account in the list,
    # ignoring SendUsingAccount. We move the target account to position 1
    # before sending, then restore the original order immediately after.
    $senderAddress = ""
    ...
    for ($i = 1; $i -le $accounts.Count; $i++) {
        Write-Host "INFO: Found account: $($accounts.Item($i).SmtpAddress)"
        if ($accounts.Item($i).SmtpAddress -eq $senderAddress) {
    ...
    if ($null -eq $account) {
        throw "ERROR: Account '$senderAddress' not found in Outlook. Please add it and try again."
    }
...
<body>
    ...
    Please contact the UUI finance office if you have any questions regarding this notice.<br><br>

    Sincerely,<br>
    <span style="font-size:20px;color:purple;font-weight:bold">Union University International<br></span>
...
"@
    ...
    # Outlook COM always sends from the first account in the account list,
    # regardless of SendUsingAccount. Verify the correct account is first,
    # and abort with a clear message if not.
    $senderAddress = "finance@uuc.edu"
    $accounts      = $namespace.Accounts
    $account       = $null

    for ($i = 1; $i -le $accounts.Count; $i++) {
        Write-Host "INFO: Found account [$i]: $($accounts.Item($i).SmtpAddress)"
        if ($accounts.Item($i).SmtpAddress -eq $senderAddress) {
            $account = $accounts.Item($i)
        }
    }
    ...
    $firstAccount = $accounts.Item(1).SmtpAddress
    if ($firstAccount -ne $senderAddress) {
        throw "ERROR: Outlook will send from '$firstAccount' (the first account) instead of '$senderAddress'.`nPlease go to Outlook → File → Account Settings → Account Settings, select '$senderAddress', and click 'Set as Default'."
    }

    Write-Host "INFO: Verified sender account is first: $senderAddress"
